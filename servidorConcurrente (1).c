...
int respuesta, conexion, conexionActual, longitudDireccion, i, frase, proceso;
			...
			if(listen(conexion, CLIENTES) == -1)
				...
				while(1)
					...
					if(conexionActual == -1)
						...
						{
						proceso = fork();
						//Se creo correctamente el hijo
						if(proceso == -1)
							{
							printf("Error Fatal: El servicio no pudo ser atendido concurretemente...\n");
							cierraSockets();
							}
						else
							{
